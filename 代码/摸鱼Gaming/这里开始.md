...
# 其他
## 戳一下 /poke
完成登录后你可以通过 Token 和 Session 来向 `/poke` 发送一个 `GET` 请求，服务器会返回用户名称、Token 有效期和 Session 有效期。这个接口主要用来简单验证 Token 和 Session 的有效。

## 账户修改
账户的修改密码、修改邮箱等操作，详见 API 文档。
修改邮箱后需要重新验证邮箱。
